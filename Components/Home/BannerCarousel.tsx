import {Dimensions, Pressable, View, Image, StyleSheet} from 'react-native';
...
const BannerCarousel: React.FC<IProps> = ({
  ...
  isLooped = false,
  height = '100%',
  itemsToShow = 1, // Default to showing 1 item
}) => {
  ...
  // Calculate the item width based on the number of items to show
    ...
    <View style={styles.container}>
      <Carousel
        ...
        renderItem={({item, index}) => (
          <Pressable
            style={[styles.bannerContainer, {aspectRatio: imageAspectRatio}]}
            key={`${item}_${index}`}>
            <Image
              style={[styles.bannerImage, {height: height}]}
              source={{uri: `${item}`}}
              // resizeMode={'contain'}
              defaultSource={{
